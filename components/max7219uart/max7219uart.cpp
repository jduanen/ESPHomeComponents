...
namespace esphome {
namespace max7219uart {

static const char *const TAG = "max7219UART";
...
void MAX7219Component::setup() {
  ESP_LOGCONFIG(TAG, "Setting up MAX7219_UARTS...");
  this->spi_setup();
  this->stepsleft_ = 0;
...
uint8_t MAX7219Component::printuart(const char *str) { return this->printuart(0, str); }

uint8_t MAX7219Component::printuart(uint8_t start_pos, const char *s) {
...
uint8_t MAX7219Component::printuartf(uint8_t pos, const char *format, ...) {
  va_list arg;
  va_start(arg, format);
  char buffer[64];
  ...
  va_end(arg);
  if (ret > 0)
    return this->printuart(pos, buffer);
  return 0;
}
uint8_t MAX7219Component::printuartf(const char *format, ...) {
  va_list arg;
  va_start(arg, format);
  char buffer[64];
  int ret = vsnprintf(buffer, sizeof(buffer), format, arg);
  va_end(arg);
  if (ret > 0)
    return this->printuart(buffer);
  return 0;
}

uint8_t MAX7219Component::strftimeuart(uint8_t pos, const char *format, ESPTime time) {
  char buffer[64];
...
uint8_t MAX7219Component::strftimeuart(const char *format, ESPTime time) {
  return this->strftimeuart(0, format, time);
}
