...
class MAX7219Component : public display::DisplayBuffer,
 ...
 public:
  ...
  uint8_t printuartf(uint8_t pos, const char *format, ...) __attribute__((format(printf, 3, 4)));
  /// Evaluate the printf-format and print the result at position 0.
  uint8_t printuartf(const char *format, ...) __attribute__((format(printf, 2, 3)));

  /// Print `str` at the given position.
  uint8_t printuart(uint8_t pos, const char *str);
  /// Print `str` at position 0.
  uint8_t printuart(const char *str);

  /// Evaluate the strftime-format and print the result at the given position.
  uint8_t strftimeuart(uint8_t pos, const char *format, ESPTime time) __attribute__((format(strftime, 3, 0)));

  /// Evaluate the strftime-format and print the result at position 0.
  uint8_t strftimeuart(const char *format, ESPTime time) __attribute__((format(strftime, 2, 0)));
